...
public class MessageView {
    ...
    private Message.RecipientStatus getMessageStatus(Message msg) {
        ...
        if (!msg.getSender().getUserId().equalsIgnoreCase(Constants.USER_NAME))
        ...
        for (int i = 0; i < ChatUsers.getAllParticipants().size(); i++) {

            //Don't check the status of the current user
            String participant = ChatUsers.getAllParticipants().get(i);
            if (participant.equalsIgnoreCase(""+Constants.USER_ID))
                continue;
